...
export default function CustomerTables() {
  ...
  const { branchId, currentBranch, availableBranches, switchBranch } = useTenantStore();
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <PageHeader
          ...
          description={`Explore seating and live tables at ${currentBranch?.name || 'Main Hall'}`}
        />
      ...
      <section className="rounded-3xl border border-neutral-200/80 bg-gradient-to-b from-white to-neutral-50/80 p-5 shadow-sm dark:border-neutral-800 dark:from-neutral-900 dark:to-neutral-950">
        <div className="mb-4 flex flex-col gap-1">
          <h2 className="text-lg font-bold tracking-tight text-neutral-900 dark:text-white sm:text-xl">
            Available Branches & Dining Outlets 🏪
          </h2>
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            Select your dining outlet to view its live table layout and seating
          </p>
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    {branch.address && (
                      <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">📍 {branch.address}</p>
                    )}
                  </div>
                  <span
                    className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${
                      isSelected ? 'bg-emerald-500 text-white' : 'bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300'
                    }`}
                  >
