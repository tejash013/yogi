...
async function ensureSubscription(restaurantId, ownerId) {
...
async function ensureStarterPlan() {
    return SubscriptionPlan.findOneAndUpdate({ key: 'starter' }, {
        $setOnInsert: {
            name: 'Starter',
            key: 'starter',
...
router.get('/current', authenticate, async (req, res) => {
    if (!['owner', 'platformAdmin'].includes(req.user.role))
...
router.get('/restaurants', authenticate, requireRole('platformAdmin'), async (_req, res) => {
    ...
    const subscriptions = await RestaurantSubscription.find({})
        ...
        .lean()
        .exec();
    const byRestaurant = new Map(subscriptions.map((item) => [String(item.restaurantId?._id ?? item.restaurantId), item]));
...
router.patch('/restaurants/:restaurantId', authenticate, requireRole('platformAdmin'), async (req, res) => {
    ...
    if (!parsedId.success || !parsedBody.success)
        return res.status(400).json(failure('Invalid subscription update'));
    ...
    if (!owner)
        return res.status(400).json(failure('Restaurant owner not found'));
    ...
    if (parsedBody.data.planId) {
        ...
        if (!plan)
            return res.status(400).json(failure('Active subscription plan not found'));
...
router.post('/plans', authenticate, requireRole('platformAdmin'), async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json(failure('Invalid subscription plan'));
