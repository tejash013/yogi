...
const localPartHasLetter = (email) => {
...
router.post('/refresh', validateBody(z.object({ refreshToken: z.string().min(1).optional() }).strict()), async (req, res) => {
    ...
    try {
        ...
        const stored = await RefreshToken.findOneAndUpdate({ token: hashRefreshToken(refreshToken), revoked: false, expiresAt: { $gt: new Date() } }, { $set: { revoked: true, revokedAt: new Date() } }, { new: true }).exec();
        if (!stored) {
        ...
        if (!user || !isSupportedRole(user.role) || user.status !== 'active')
            return res.status(401).json(failure('Invalid refresh token'));
...
router.post('/reset-password', validateBody(resetSchema), async (req, res) => {
    ...
    const updated = await User.findOneAndUpdate({ _id: user._id, resetToken: hashResetToken(token), resetTokenExpires: { $gt: new Date() } }, { password: hashPassword(password), $inc: { tokenVersion: 1 }, $unset: { resetToken: 1, resetTokenExpires: 1 } }, { new: true }).exec();
    if (!updated)
        return res.status(400).json(failure('Invalid or expired reset token'));
