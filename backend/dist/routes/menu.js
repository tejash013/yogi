...
async function resolveMenuImage(image, tenant) {
    ...
    const hasCloudinaryConfig = Boolean(process.env.CLOUDINARY_URL || (process.env.CLOUDINARY_CLOUD_NAME &&
        process.env.CLOUDINARY_API_KEY &&
        process.env.CLOUDINARY_API_SECRET));
    if (hasCloudinaryConfig) {
        try {
            const result = await uploadImage(trimmed, undefined, `restaurants/${tenant.restaurantId}/branches/${tenant.branchId}/menu-items`);
...
router.patch('/:id', authenticate, requirePermission(permissions.menuUpdate), validateParams(idParamSchema), validateBody(menuUpdateSchema), async (req, res) => {
    try {
        ...
        if (!item && (req.user?.role === 'owner' || req.user?.role === 'platformAdmin')) {
