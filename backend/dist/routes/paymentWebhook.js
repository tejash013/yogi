...
router.post('/', async (req, res) => {
    ...
    if (!secret || !signature || !eventId || !Buffer.isBuffer(req.body))
        return res.status(401).json(failure('Invalid webhook authentication'));
    ...
    try {
        ...
        return res.status(400).json(failure('Invalid webhook payload'));
    }
    if (!event.invoiceId || !['paid', 'cancelled'].includes(event.status) || !event.transactionId || !Number.isFinite(Number(event.amount)))
        return res.status(400).json(failure('Invalid webhook event'));
    ...
    try {
        ...
        if (error?.code === 11000)
            return res.json(success(null, 'Webhook already processed'));
        throw error;
    }
    const invoice = await Invoice.findOneAndUpdate({ _id: event.invoiceId, ...tenant, amount: Number(event.amount), status: { $nin: ['paid', 'cancelled'] } }, { status: event.status, transactionId: event.transactionId, ...(event.status === 'paid' ? { paidAt: new Date() } : {}) }, { new: true, runValidators: true }).exec();
    if (!invoice) {
