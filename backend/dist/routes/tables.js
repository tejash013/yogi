...
const router = Router();
function emitTableEvent(event, table) {
    try {
        const io = getIO();
...
router.post('/:id/reserve', authenticate, validateParams(idParamSchema), async (req, res) => {
    const table = await Table.findOneAndUpdate({ _id: req.params.id, ...tenantFilter(req), status: { $in: ['available', 'reserved'] } }, { status: 'reserved' }, { new: true }).exec();
    if (!table) {
...
router.patch('/:id/status', authenticate, validateParams(idParamSchema), validateBody(tableStatusSchema), async (req, res) => {
