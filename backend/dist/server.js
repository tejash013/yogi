...
async function startServer() {
    try {
        ...
        const migratedPlatformAdmins = await User.updateMany({ role: { $in: ['platformadmin', 'platform-admin'] } }, { $set: { role: 'platformAdmin' } });
        ...
        const migratedMenuItems = await MenuItem.updateMany({ availableQty: { $exists: true } }, { $unset: { availableQty: 1 } });
