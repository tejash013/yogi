...
export async function seedDatabase() {
    ...
    await Category.updateMany({ $or: [{ restaurantId: { $exists: false } }, { branchId: { $exists: false } }] }, { $set: tenant }).catch(() => undefined);
    await MenuItem.updateMany({ $or: [{ restaurantId: { $exists: false } }, { branchId: { $exists: false } }] }, { $set: tenant }).catch(() => undefined);
    await Offer.updateMany({ $or: [{ offerType: { $ne: 'coupon' }, code: { $exists: true } }, { offerType: 'coupon', code: { $in: [null, ''] } }] }, { $unset: { code: '' } }).catch(() => undefined);
