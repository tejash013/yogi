...
export function attachSocketHandlers(server) {
    ...
    io.on('connection', (socket) => {
        ...
        if (socket.data.user.role !== 'customer')
            socket.join(`${tenant}:staff:orders`);
        socket.emit('connected', { ok: true });
        socket.on('join', (room) => {
            if (typeof room !== 'string' || !room || room.length > 100)
                return;
            if (room === `user:${socket.data.user.restaurantId}:${socket.data.user.branchId}:user:${socket.data.user.id}`)
                socket.join(room);
        });
