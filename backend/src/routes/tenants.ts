...
function withResolvedCoords<T extends {
  address?: string;
  name?: string;
  latitude?: number;
  longitude?: number;
  addressDetails?: { city?: string; state?: string; pincode?: string; street?: string };
}>(item: T): T {
...
router.put('/restaurants/:id', authenticate, requireRole(['platformAdmin', 'owner']), validateParams(idParamSchema), validateBody(restaurantUpdateSchema), async (req: any, res) => {
  ...
  if (!updated) return res.status(404).json(failure('Restaurant not found'));
...
router.delete('/restaurants/:id', authenticate, requireRole(['platformAdmin', 'owner']), validateParams(idParamSchema), async (req: any, res) => {
  ...
  if (permanent) {
    ...
    if (!deleted) return res.status(404).json(failure('Restaurant not found'));
  ...
  const deactivated = await Restaurant.findByIdAndUpdate(
    req.params.id,
    { $set: { isActive: false } },
    { new: true }
  ).exec();
  if (!deactivated) return res.status(404).json(failure('Restaurant not found'));
...
router.get('/branches/:id', optionalAuth, validateParams(idParamSchema), async (req, res) => {
  ...
  if (!branch) return res.status(404).json(failure('Branch not found'));
...
router.post('/restaurants/:id/branches', authenticate, requireRole(['platformAdmin', 'owner']), validateParams(idParamSchema), validateBody(branchSchema), async (req: any, res) => {
...
router.put('/branches/:id', authenticate, requireRole(['platformAdmin', 'owner', 'manager']), validateParams(idParamSchema), validateBody(branchUpdateSchema), async (req: any, res) => {
...
router.delete('/branches/:id', authenticate, requireRole(['platformAdmin', 'owner']), validateParams(idParamSchema), async (req: any, res) => {
  const branch = await Branch.findById(req.params.id).exec();
  if (!branch) return res.status(404).json(failure('Branch not found'));
  ...
  const deactivated = await Branch.findByIdAndUpdate(
    req.params.id,
    { $set: { isActive: false } },
    { new: true }
  ).exec();
