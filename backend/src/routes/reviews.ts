...
router.post('/', authenticate, requirePermission(permissions.reviewsCreate), validateBody(reviewCreateSchema), async (req: any, res) => {
  ...
  if (!menuItem) return res.status(404).json(failure('Menu item not found'));

  const review = await Review.findOneAndUpdate(
    { menuItem: menuItemId, user: req.user.id, ...tenant },
    { menuItem: menuItemId, user: req.user.id, rating, subject, comment, images: images ?? [], ...tenant },
