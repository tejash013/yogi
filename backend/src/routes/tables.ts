...
router.get('/qr/:token', optionalAuth, async (req, res) => {
  ...
  if (!/^[a-f0-9]{32}$/i.test(token)) return res.status(404).json(failure('QR code not found'));
  const table = await Table.findOne({ qrTokenHash: hashQrToken(token) }).lean().exec();
  if (!table) return res.status(404).json(failure('QR code not found or expired'));
...
router.post('/:id/qr-token', authenticate, requirePermission(permissions.tablesManage), validateParams(idParamSchema), async (req: any, res) => {
  const token = randomBytes(16).toString('hex');
  const table = await Table.findOneAndUpdate(
    { _id: req.params.id, ...tenantFilter(req) },
    { qrTokenHash: hashQrToken(token), qrTokenIssuedAt: new Date() },
    { new: true },
  ).lean().exec();
  if (!table) return res.status(404).json(failure('Table not found'));
  return res.json(success({
    tableId: String(table._id),
    label: table.label,
    token,
...
router.delete('/:id/qr-token', authenticate, requirePermission(permissions.tablesManage), validateParams(idParamSchema), async (req: any, res) => {
  const table = await Table.findOneAndUpdate(
    { _id: req.params.id, ...tenantFilter(req) },
    { $unset: { qrTokenHash: 1, qrTokenIssuedAt: 1 } },
    { new: true },
  ).lean().exec();
  if (!table) return res.status(404).json(failure('Table not found'));
