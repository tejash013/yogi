...
router.get('/', authenticate, requirePermission(permissions.invoicesRead), validateQuery(invoiceQuerySchema), async (req, res) => {
  ...
  if (isCustomer(req)) {
    const orderIds = await Order.find({ user: (req as any).user.id, ...tenantFilter(req) }).distinct('_id').exec();
...
router.get('/:id', authenticate, requirePermission(permissions.invoicesRead), validateParams(idParamSchema), async (req, res) => {
  ...
  if (isCustomer(req) && String(order?.user) !== (req as any).user.id) return res.status(403).json(failure('You do not have access to this invoice'));
...
router.post('/', authenticate, requirePermission(permissions.invoicesCreate), validateBody(invoiceCreateSchema), async (req, res) => {
  ...
  if (!idempotencyKey || idempotencyKey.length > 128) return res.status(400).json(failure('A valid Idempotency-Key header is required'));
  if (!orderId) {
  ...
  if (isCustomer(req) && String(order.user) !== (req as any).user.id) return res.status(403).json(failure('You do not have access to this order'));

  const existing = await Invoice.findOne({ ...tenant, $or: [{ order: orderId }, { idempotencyKey }] }).exec();
  if (existing) return res.status(200).json(success(existing, 'Invoice already exists'));
...
router.patch('/:id', authenticate, requirePermission(permissions.invoicesUpdate), validateParams(idParamSchema), validateBody(invoiceUpdateSchema), async (req, res) => {
  ...
  await recordAudit({ actor: (req as any).user.id, action: 'invoice.payment_method_updated', resourceType: 'Invoice', resourceId: String(invoice._id), metadata: { paymentMethod: req.body.paymentMethod }, ip: req.ip, userAgent: req.get('user-agent') });
...
router.patch('/:id/status', authenticate, requirePermission(permissions.invoicesUpdate), validateParams(idParamSchema), validateBody(invoiceStatusSchema), async (req, res) => {
  ...
  if (status === 'paid' && !transactionId) return res.status(400).json(failure('transactionId is required when marking an invoice paid'));

  const invoice = await Invoice.findOneAndUpdate(
    { _id: req.params.id, ...tenantFilter(req), status: { $nin: ['paid', 'cancelled'] } },
    { status, transactionId, ...(status === 'paid' ? { paidAt: new Date() } : {}) },
  ...
  await recordAudit({ actor: (req as any).user.id, action: 'invoice.status_updated', resourceType: 'Invoice', resourceId: String(invoice._id), metadata: { status }, ip: req.ip, userAgent: req.get('user-agent') });
