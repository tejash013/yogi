...
export function tenantIdsFromRequest(req: any) {
  let restaurantId: any = req.query?.restaurantId ?? req.headers['x-restaurant-id'] ?? req.user?.restaurantId ?? DEFAULT_RESTAURANT_ID;
  let branchId: any = req.query?.branchId ?? req.headers['x-branch-id'] ?? req.user?.branchId ?? DEFAULT_BRANCH_ID;
